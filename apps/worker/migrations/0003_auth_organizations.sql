...
create table "team" ("id" text not null primary key, "name" text not null, "memberCount" integer not null, "organizationId" text not null references "organization" ("id") on delete cascade, "createdAt" date not null, "updatedAt" date);
...
-- Domain bridge: Better Auth owns organization membership while Home OS keeps
-- its existing household foreign keys. The first organization may claim the
-- legacy `home` row; later organizations receive their own household row.
alter table "households" add column "organization_id" text references "organization" ("id") on delete set null;

create unique index "households_organization_id_uidx" on "households" ("organization_id");
